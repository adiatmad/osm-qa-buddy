...
function taskStyle(feature) {{
  const n = Number(feature.properties?.qa_total_issues || 0);
...
const taskLayer = L.geoJSON(tasks, {{
  ...
  onEachFeature: (feature, layer) => {{
    ...
    layer.bindPopup('<b>Task:</b> '+id+'<br><b>Findings:</b> '+findings+'<br><b>Unique OSM objects:</b> '+objects+'<br><b>Rules:</b> '+escapeHtml(rules)+'<br><b>Errors:</b> '+(p.qa_error_count||0)+'<br><b>Warnings:</b> '+(p.qa_warning_count||0));
  }}
}}).addTo(map);
...
legend.onAdd = function() {{ const div=L.DomUtil.create('div','legend'); div.innerHTML='<b>Task findings</b><br><span class="swatch" style="background:#2ca25f"></span>0<br><span class="swatch" style="background:#fee08b"></span>1–4<br><span class="swatch" style="background:#fdae61"></span>5–9<br><span class="swatch" style="background:#d73027"></span>10+'; return div; }};
legend.addTo(map);
...
def generate_report(errors_path, tasks_path, report_path, map_path):
    ...
    total_findings = sum((f.get("properties", {}).get("qa_finding_count") or f.get("properties", {}).get("qa_total_issues") or 0) for f in task_features)
...
<div class="note"><b>Important:</b> The raw <code>qa_errors.geojson</code> contains one feature per reported JOSM finding. Unique-object and rule metrics are derived per task and are shown in the task summary.</div>
...
<h2>Outputs</h2><p><a href="qa_errors.geojson">Raw QA findings GeoJSON</a><br><a href="task_grid_qa_summary.geojson">Task grid QA summary GeoJSON</a></p>
</body></html>'''
