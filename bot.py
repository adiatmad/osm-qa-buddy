...
WORK_DIR = os.environ.get("QABOT_WORK_DIR", os.path.abspath("work"))
...
try:
    ...
    custom_mapcss_path = os.path.join(WORK_DIR, "size_rule.mapcss")
    ...
    target_file = os.path.join(WORK_DIR, "sample.osm")
    ...
    print("\nStarting validation process (proven JOSM headless test loop)...")
    sys.stdout.flush()
    all_errors = []
    ...
    for err in all_errors:
        try:
            ...
            geojson_output["features"].append({"type": "Feature", "geometry": {"type": "Point", "coordinates": [lon, lat]}, "properties": {"rule": test_name, "message": message, "severity": severity, "object_id": obj_id_str, "josm_remote_url": josm_url}})
        except Exception:
            pass

    geojson_path = os.path.join(WORK_DIR, "qa_errors.geojson")
