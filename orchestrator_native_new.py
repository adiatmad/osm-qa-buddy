...
def run_josm_qa_bot(ram_gb):
    ...
    cmd = ["java", f"-Xmx{ram_gb}g", "-cp", classpath, "org.python.util.jython", str(REPO_DIR / "bot.py")]
...
def aggregate_errors_to_tasks(tasks_geojson_path, errors_geojson_path):
    ...
    for task_feat in tasks_data.get("features", []):
        ...
        task_geometries.append(task_poly)
        ...
        unique_objects = {x["object_id"] for x in findings if x["object_id"]}
        rules = {x["rule"] for x in findings if x["rule"]}
        unique_findings = {_finding_key(x) for x in findings}
        ...
        props.update({
            "qa_task_status": task_status or "UNKNOWN",
            "qa_badimagery": badimagery,
            "qa_finding_count": len(findings),
            "qa_unique_finding_count": len(unique_findings),
            "qa_unique_osm_object_count": len(unique_objects),
            "qa_rules_involved": sorted(rules),
            "qa_error_count": error_count,
            "qa_warning_count": warning_count,
            "qa_unknown_severity_count": len(findings) - error_count - warning_count,
            "qa_total_issues": len(findings),
            "qa_priority": _priority_for_task(len(findings), len(unique_findings), len(unique_objects), badimagery),
        })
    ...
    tasks_data["qa_summary"] = {
        ...
        "unassigned_finding_count": len(unassigned),
        "task_associated_unique_finding_count": len({_finding_key(error) for poly in task_geometries for error in error_points if poly.covers(error["point"])}),
        "priority_definition": {"HIGH": "BADIMAGERY or at least 5 unique findings or 5 unique OSM objects", "MEDIUM": "2-4 unique findings or 2-4 unique OSM objects", "LOW": "1 raw finding with no HIGH/MEDIUM signal", "NONE": "No QA finding signal and not BADIMAGERY"},
        "human_review_required": True,
    }
...
def run_local_pipeline(pbf_path, aoi_path, tasks_path, output_dir=None, ram_gb=24):
    ...
    write_run_metadata(aoi_path, tasks_path, pbf_path, metadata_path, run_started_utc, ram_gb)
